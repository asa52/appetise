...
class Ingredient(BaseModel):
    ...
    """
    name: str = Field(..., description="The common name of the ingredient (e.g., 'Flour', 'Milk').")
    quantity: float = Field(..., gt=0, description="The numerical amount of the ingredient.")
    unit: str = Field(..., description="The measurement unit (e.g., 'grams', 'ml', 'cups', 'teaspoons').")
...
class Step(BaseModel):
    ...
    """
    order: int = Field(..., ge=1, description="The sequential order of the step in the recipe.")
...
class Recipe(BaseModel):
    ...
    """
    recipe_id: Optional[str] = Field(None, description="Unique identifier for the recipe (used for database storage).")
    name: str = Field(..., description="The title of the recipe.")
    description: Optional[str] = Field(None, description="A brief description or summary of the dish.")
    ingredients: List[Ingredient] = Field(..., description="A list of ingredients required for the recipe.")
    steps: List[Step] = Field(..., description="The sequential list of steps to follow.")
    prep_time_minutes: Optional[int] = Field(None, ge=0, description="Estimated preparation time in minutes.")
    cook_time_minutes: Optional[int] = Field(None, ge=0, description="Estimated cooking time in minutes.")
...
class InventoryItem(Ingredient):
    ...
    inventory_id: Optional[str] = Field(None, description="Unique identifier for this inventory record.")
    storage_location: Optional[str] = Field(None, description="Where the item is stored (e.g., 'Fridge', 'Pantry').")
